...
<link href="https://fonts.googleapis.com/css2?family=Fraunces:ital,opsz,wght@0,9..144,400..900;1,9..144,400..900&family=DM+Sans:wght@400;500;600;700&display=swap" rel="stylesheet">
<style>
  :root{
  ...
  *{box-sizing:border-box;}
  ...
  body{font-family:"DM Sans",system-ui,sans-serif;color:var(--ink);background:#d7ecea;}
  ...
  .card{background:var(--paper);height:100%;border-radius:4mm;overflow:hidden;
    display:flex;flex-direction:column;position:relative;}

  /* brand bar */
  .brand{background:var(--teal);display:flex;align-items:center;justify-content:space-between;
    padding:5mm 8mm;-webkit-print-color-adjust:exact;print-color-adjust:exact;}
  .brand img{height:9mm;width:auto;}
  .pill{font-size:11px;font-weight:700;letter-spacing:1.4px;text-transform:uppercase;
    background:var(--gold);color:#3b2c00;padding:5px 14px;border-radius:999px;
  ...
  .name{font-family:"Fraunces",serif;font-optical-sizing:auto;font-weight:600;font-style:italic;
    font-size:46px;line-height:.95;letter-spacing:-.5px;color:var(--ink);margin:0;}
  ...
  /* story */
  .story{padding:4mm 8mm 5mm;flex:1 1 auto;font-size:13.5px;line-height:1.62;color:#33403e;}
  .story p{margin:0 0 9px;}
  .story p:last-child{margin-bottom:0;}
  .story p:first-of-type::first-letter{font-family:"Fraunces",serif;font-weight:600;font-style:italic;
    font-size:3.1em;line-height:.74;float:left;margin:3px 9px 0 0;color:var(--teal-deep);}

  /* footer CTA */
  .cta{margin-top:auto;background:var(--teal);color:#fff;display:flex;align-items:center;justify-content:space-between;
    gap:6mm;padding:6mm 8mm;-webkit-print-color-adjust:exact;print-color-adjust:exact;}
  .cta .left{display:flex;align-items:center;gap:4mm;min-width:0;}
  .cta .heart{font-size:24px;color:var(--coral);line-height:1;-webkit-print-color-adjust:exact;print-color-adjust:exact;}
  .cta .lead{font-family:"Fraunces",serif;font-style:italic;font-weight:600;font-size:18px;line-height:1.1;}
  .cta .url{display:block;font-size:11.5px;color:rgba(255,255,255,.85);letter-spacing:.2px;margin-top:1mm;
    overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:95mm;}
  .cta .mail{text-align:right;font-size:11px;line-height:1.5;color:rgba(255,255,255,.92);flex:0 0 auto;}
  .cta .mail b{display:block;font-size:12.5px;color:#fff;}
    ...
    <div class="card">
      ...
      <footer class="cta">
        <div class="left">
          ...
          <div>
            <span class="lead" id="lead">Sponsor me</span>
            <a class="url" id="url" href="#"></a>
          </div>
        </div>
        <div class="mail">write to<br><b><a href="mailto:info@simabo.org">info@simabo.org</a></b></div>
      </footer>
    </div>
  </div>
</div>
...
(function(){
  ...
  function render(a){
    ...
    const name=a.name||'this animal';
    document.getElementById('lead').textContent=`Sponsor ${name}`;
    const url=document.getElementById('url');
    if(a.link){url.href=a.link;url.textContent=a.link.replace(/^https?:\/\//,'');}
  ...
  function fit(){
    ...
    let guard=0;
    while(card.scrollHeight>card.clientHeight+1 && size>9 && guard<50){
